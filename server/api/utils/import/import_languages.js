...
var fs2 = require('fs');
var outputFile = './importFiles/language.json'

var instream = fs.createReadStream('../../../stage/language.list', {encoding: 'binary'});
    ...
    if (line.length > 0) {
      ...
      if (line.substring(0, 4) === '----') {
        ...
        if(checkForMisc !== ''  && checkForMisc !== titleHold) {
          newRecord.language = checkForMisc.trim().replace(/"/g, '\\"');
          newRecord.misc = parseArray[parseArray.length-1].trim().replace(/"/g, '\\"');
        } else {
          newRecord.language = parseArray[parseArray.length-1].trim().replace(/"/g, '\\"');
        }
...
// mongoimport --db imdbproject-dev --collection language --file language.json
