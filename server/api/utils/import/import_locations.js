...
var fs2 = require('fs');
var outputFile = './importFiles/locations.json'

var instream = fs.createReadStream('../../../stage/locations.list', {encoding: 'binary'});
    ...
    if (line.length > 0) {
      ...
      if (line.substring(0, 4) === '----') {
        ...
        if(checkForMisc !== ''  && checkForMisc !== titleHold) {
          newRecord.location = checkForMisc.trim().replace(/"/g, '\\"');
          newRecord.misc = parseArray[parseArray.length-1].trim().replace(/"/g, '\\"');
        } else {
          newRecord.location = parseArray[parseArray.length-1].trim().replace(/"/g, '\\"');
        }
...
// mongoimport --db imdbproject-dev --collection locations --file locations.json
