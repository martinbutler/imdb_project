...
var outputFile = './importFiles/cinematographers.json';

var instream = fs.createReadStream('../../../stage/cinematographers.list', {encoding: 'binary'});
...
rl.on('line', function(line) {
  if (atData) {
    ...
    if (line.length > 0) {
      ...
      if (line.substring(0, 4) === '----') {
        ...
        // check if line is includes cinematographers name
        if (parseArray[0] !== '') {
  ...
  } else if(line === "THE CINEMATOGRAPHERS LIST") {
    atTitle = true;
...
// mongoimport --db imdbproject-dev --collection cinematographers --file cinematographers.json
