...
var outputFile = './importFiles/directors.json';

var instream = fs.createReadStream('../../../stage/directors.list', {encoding: 'binary'});
  ...
  if (atData) {
    ...
    if (line.length > 0) {
      ...
      if (line.substring(0, 4) === '----') {
        ...
        // check if line includes director's name
        if (parseArray[0] !== '') {
        ...
        if (additionalInfoMatch = fullTitleData.substring(newRecord.title.length).trim()) {
            newRecord.additionalInfo = fullTitleData.substring(newRecord.title.length).trim().replace(/"/g, '\\"');
        }
        fs.appendFileSync(outputFile, JSON.stringify(newRecord) + "\n");
        delete newRecord.title;
        delete newRecord.additionalInfo;
      }
    }
  // logic to skip non movie data at the head of the file
  } else if(line === "THE DIRECTORS LIST") {
    atTitle = true;
...
// mongoimport --db imdbproject-dev --collection directors --file directors.json
