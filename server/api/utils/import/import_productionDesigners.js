...
var outputFile = './importFiles/productionDesigners.json';

var instream = fs.createReadStream('../../../stage/production-designers.list', {encoding: 'binary'});
  ...
  if (atData) {
    ...
    if (line.length > 0) {
      ...
      if (line.substring(0, 4) === '----') {
        ...
        // check if line is includes designers name
        if (parseArray[0] !== '') {
        ...
        //   most records will have the no designer name data
        ...
        // parse misc
        var miscMatch = fullTitleData.substring(newRecord.title.length).trim().replace(/"/g, '\\"');
        if (miscMatch) {
          newRecord.misc = miscMatch;  
        }
        fs.appendFileSync(outputFile, JSON.stringify(newRecord) + "\n");
        var designers = newRecord.name;
        newRecord = {};
        newRecord.name = designers;
      }
    }
  // logic to skip non movie data at the head of the file
  } else if(line === "THE PRODUCTION DESIGNERS LIST") {
    atTitle = true;
...
// mongoimport --db imdbproject-dev --collection productiondesigners --file productionDesigners.json
